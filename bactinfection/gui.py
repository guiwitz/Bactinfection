...
class Gui(Bact):
    ...
    def __init__(self):
        ...
        self.nucl_channel_seg = ipw.Select(options = self.channel_field.value.replace(' ','').split(','),
                                     layout = {'width': '200px'},style = {'description_width': 'initial'}, value = None)
        ...
        with self.out:
            ...
            if totfiles == 0:
                ...
                if len(self.channels)>self.current_image.shape[2]:
                    ...
                    self.analyze_button.description = 'Run segmentation'
                elif (self.nucl_channel_seg.value is None) or (self.bact_channel_seg.value is None):
                    ...
                    for ind, f in enumerate(self.all_files):
                        ...
                        valid = self.run_analysis(self.nucl_channel_seg.value,self.bact_channel_seg.value, self.folder_name+'/'+f)
                        if not valid:
                            break
            self.analyze_button.description = 'Run segmentation'
